...
static void spe_prune_stale(void);
...
/*
 * damon_spe_record_access - Record a single subpage access from SPE trace
 * @pfn:	Physical page frame number (can be any page within a THP)
 ...
 *
 * This is called either from:
 *   (a) the kernel SPE overflow handler (Phase 2c), or
 *   (b) a userspace daemon via debugfs/sysfs (Phase 2b).
 *
 * Context: can be called from IRQ context (holds spe_lock with GFP_ATOMIC).
 */
 ...
 *
 * First tries the SPE rbtree (accumulated from SPE sampling).
 * Falls back to PTE access bits if no SPE data is available.
 *
 * Returns the number of chunks on success, negative error otherwise.
 */
			    ...
			    unsigned long *hot_bitmap)
{
	...
	if (e && e->total_accesses > 0) {
		...
		for (i = 0; i < num_chunks; i++) {
			...
			int j;
			/*
			 * Aggregate subpage counts within each target_order
			 * chunk.  A chunk at target_order covers
			 * (chunk_sz / PAGE_SIZE) subpages.
			 */
...
/*
 * damon_spe_prune - Periodic aging of stale entries
 *
 * Called from DAMON's aggregation cycle (or a periodic timer).
 * Removes entries that haven't been updated within DAMON_SPE_ENTRY_TTL.
 */
...
/*
 * damon_spe_stats - Return current SPE statistics
 *
 * For debugging / sysfs reporting.
 */
...
static int spe_stats_show(struct seq_file *m, void *v)
{
	...
	/* Show top entries */
	spin_lock_irqsave(&spe_lock, flags);
	for (node = rb_first(&spe_tree); node && count > 0; node = rb_next(node)) {
		struct damon_spe_entry *e =
			rb_entry(node, struct damon_spe_entry, node);
